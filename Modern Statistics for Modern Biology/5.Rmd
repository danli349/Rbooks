...
head(scale(simdatxy, scale = FALSE)) # subtract the center, not scale


```

```{r}
mean(scale(simdatxy, scale = FALSE)[,1])
mean(scale(simdatxy, scale = FALSE)[,2])
```
...
```{r}
library("mixtools")
dim(rmvnorm(n = 1e3, mu = -c(1, 1), sigma = 0.5 * diag(c(1, 1))))
mean(rmvnorm(n = 1e3, mu = -c(1, 1), sigma = 0.5 * diag(c(1, 1)))[,1])
mean(rmvnorm(n = 1e3, mu = -c(1, 1), sigma = 0.5 * diag(c(1, 1)))[,2])
```
...
```{r}
10 ^ seq(log10(2), log10(2000), length.out = 20)
```
